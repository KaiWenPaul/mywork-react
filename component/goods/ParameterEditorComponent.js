...
class ParameterEditorComponent extends BaseComponent{
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            parameter_pc_price:parameterBean.parameter_pc_price?parameterBean.parameter_pc_price:"",
            parameter_origin_price:parameterBean.parameter_origin_price?parameterBean.parameter_origin_price:"",
    ...
    insertMoudle(){
        ...
        if(isNaN(this.state.parameter_pc_price)){
            toast.show("价格不合法");
            return;
        }
        if(isNaN(this.state.parameter_origin_price)){
            toast.show("价格不合法");
            return;
        }
        if(!this.state.parameterBean.parameter_id){
            ...
            this.getDataByPost(2,homeurl+"goodsController.api?updateParameter",
                {parameter_id:this.state.parameterBean.parameter_id,parameter_name:this.state.parameter_name,
                    parameter_price:this.state.parameter_price,parameter_origin_price:this.state.parameter_origin_price,
                    parameter_pc_price:this.state.parameter_pc_price,
                    parent_id:this.state.parameterBean.parent_id,goods_id:this.state.parameterBean.goods_id,
                    parameter_type:this.state.parameterBean.parameter_type,
                    sort:this.state.sort});

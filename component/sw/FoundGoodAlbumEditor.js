...
class GoodsClassEditorComponent extends Widget.BaseComponent{
    ...
    insertClass() {
        ...
        params["album_url"]=this.state.albumBean.album_url;
        params["album_web_url"]=this.state.albumBean.album_web_url;
        params["ablum_img"]=this.state.albumBean.ablum_img;
        params["ablum_web_img"]=this.state.albumBean.ablum_web_img;
        ...
        return(
            <div>
                ...
                <Widget.Editor
                    marginTop={20}
                    title="网页端链接地址"
                    value={this.state.albumBean.album_web_url}
                    onChange={(value)=>{
                        this.state.albumBean.album_web_url=value;
                        this.setState({
                            albumBean:this.state.albumBean
                        })
                    }}/>
                <Widget.Editor
                    marginTop={20}
                    title="移动端链接地址"
                    value={this.state.albumBean.album_url}
                    onChange={(value)=>{
                        this.state.albumBean.album_url=value;
                        this.setState({
                            albumBean:this.state.albumBean
                        })
                    }}/>
                ...
                <div style={{display:'flex'}}>
                    <Widget.Img
                        marginTop={20}
                        title="网页端图片"
                        src={this.isNull(this.state.albumBean.ablum_web_img)?"./images/add.jpg":imgurl+this.state.albumBean.ablum_web_img}
                        url={homeurl+'goodsController.api?uploadGoodsImg'}
                        onSuccess={(data)=>{
                            this.state.albumBean.ablum_web_img=data;
                            this.setState({
                                albumBean:this.state.albumBean,
                            })
                        }}/>

...
class MerchantsEditorComponent extends Widget.BaseComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            moudleBeans: [],
            index: 0,
            is_insert:!merchantsBean?-1:(merchantsBean.merchants_id?0:-1),
        }
    }
    ...
    renderBase() {
        return (
            <MerchantsBaseDetailComponent
                ...
                is_insert ={this.state.is_insert}>
            </MerchantsBaseDetailComponent>
        );
    }
...
class MerchantsBaseDetailComponent extends Widget.BaseComponent {
    ...
    componentDidMount() {
        ...
        if(this.state.merchants_id){
            ...
            this.getDataByPost(12,homeurl+"merchantsController.api?getMerchantsStoreHouseList",{merchants_id:this.state.merchants_id});
            ...
            console.log("更正的storeHouseList:");
            console.log(this.state.storeHouseList);
        }
    }
    ...
    insertMerchants() {
        ...
        */
        ...
        var storehouse_str_new = ''
        for(var i = 0; i < this.state.storeHouseList.length ; ++i){
            var sh = this.state.storeHouseList[i]
            storehouse_str_new += sh.storehouse_province + '/' + sh.storehouse_city + '/' + sh.storehouse_area + '/' + sh.detailed_address + ','
        }
        storehouse_str_new = storehouse_str_new.substring(0,storehouse_str_new.length - 1)
        ...
        var ini_save_dic = {
             ...
             */
            storehouseStr:storehouse_str_new
        };

        // 判断storehouseStr是否变化过
        console.log(storehouse_str_new)
        if(!this.state.pre_storehouseStr){
            delete ini_save_dic["storehouseStr"]
        }

        // 判断brand_id是否变化过
        if(!this.state.merchantsBean.brandIds) {
            delete ini_save_dic["brandIds"]
        }

        if (!this.state.merchantsBean.merchants_id){
            delete ini_save_dic["merchants_id"]
            this.getDataByPost(1, homeurl + "merchantsController.api?insertMerchantDetail", ini_save_dic)
        }else{
            this.getDataByPost(1, homeurl + "merchantsController.api?updateMerchantDetail", ini_save_dic)
        }
    }
    ...
    doSuccess(index, data) {
        switch (index) {
            ...
            case 12:
                ...
                console.log("后台加载的storeHouseList:");
                console.log(this.state.storeHouseList);
                if(!this.state.storeHouseList){
                    this.setState({
                        storeHouseList: []
                    });
                    console.log("更正的storeHouseList:");
            ...
            <div>
                ...
                <div style={{display:'flex',alignItems:'center',marginTop:20}}>
                    {/*<Widget.Select
                     ...
                     detailed_address={this.state.}
                     flag={this.props.is_insert===-1?1:this.state.merchantsBean.merchants_id}
    ...
    // 遍历加载三级库存位置、街道地址
    renderStorehouseAddress() {
        if(!this.state.storeHouseList[0]){
            this.state.storeHouseList[0]={};
        }
        let data=[];
        console.log('遍历加载三级库存位置、街道地址')
        for(let i=0;i<this.state.storeHouseList.length;i++){
            //console.log("遍历："+i+"次数");
            //console.log(this.state.storeHouseList[i].storehouse_province);
            let house = this.state.storeHouseList[i];
            data.push(<Widget.Address
                title="库存位置："
                province={this.state.storeHouseList[i].storehouse_province}
                city={this.state.storeHouseList[i].storehouse_city}
                area={this.state.storeHouseList[i].storehouse_area}
                detailed_address={this.state.storeHouseList[i].detailed_address}
                flag={this.props.is_insert === -1 ? 1 : this.state.merchantsBean.merchants_id}
                name_order={i}
                onSelectChange={(pro, city, area) => {
                    this.state.storeHouseList[i].storehouse_province = pro;
                    this.state.storeHouseList[i].storehouse_city = city;
                    this.state.storeHouseList[i].storehouse_area = area;
                    this.state.pre_storehouseStr = '1'
                    this.forceUpdate();
                }}/>)
            data.push(<br/>)
            data.push(
                <Widget.Editor
                   title="街道地址："
                   value={this.state.storeHouseList[i].detailed_address}
                   onChange={(value)=>{
                       this.state.storeHouseList[i].detailed_address = value;
                       this.state.pre_storehouseStr = '1'
                       this.setState({
                           storeHouseList:this.state.storeHouseList
                       })
                   }}>/</Widget.Editor>)
            data.push(<br/>)
        }
    ...
    renderLabel(){
        return(
            <div style={{marginBottom:25,marginTop:15}}>
                <div style={{width:100,display:'flex',justifyContent:'flex-end',float:'left',height:30,lineHeight:'30px'}}>

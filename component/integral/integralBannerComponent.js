...
class BannerComponent extends Widget.BaseComponent{
    ...
    componentDidMount() {
        this.setState({
            baseData:[
                ...
                {name:"标题",flex:2,key:'banner_name'},
    ...
    getBeannrs(){
        this.getDataByPost(1,homeurl+"integralInterfaces.api?getIntegralBannerList",
            {},{type:2})
    }
    ...
    doSuccess(index,data){
        switch (index){
            case 1:
                ...
                console.log(this.state.bannerBeans)
                break;
            case 2:
                toast.show("删除成功");
                this.getBeannrs();
        ...
        this.getDataByPost(2,homeurl+"intergralController.api?saveIntegralBanner",
            {
                ...
                web_url: this.state.bannerBeans[this.state.banner_index].web_url,
                web_img: this.state.bannerBeans[this.state.banner_index].web_img,
                app_url:this.state.bannerBeans[this.state.banner_index].app_url,
                app_img: this.state.bannerBeans[this.state.banner_index].app_img,
                start_time:this.state.bannerBeans[this.state.banner_index].start_time,
                end_time: this.state.bannerBeans[this.state.banner_index].end_time,
                sort: this.state.bannerBeans[this.state.banner_index].sort,
                banner_name:this.state.bannerBeans[this.state.banner_index].banner_name,
                action_token:'d'
            });
    }
    ...
    render(){
        ...
        return(
            <div style={{background:'#ffffff',display:'flex',flexDirection:'column'}}>
                ...
                <Widget.Toolbar title={"banner列表"} history={this.props.history}></Widget.Toolbar>
                ...
                <div style={{display:'flex',alignItems:'center',justifyContent:'flex-end',marginTop:20,}}>
                    <Widget.Button
                        ...
                        value="添加"
                    onClick={()=>{
                            this.props.history.push("/integral_banner_editor/"+encodeURIComponent(JSON.stringify({})));
                        }}/>
                </div>
                <Widget.ListViewComponent
                    ...
                    operationClick={(rowID,index)=>{
                        switch (index){
                            case 0:
                                this.props.history.push("/integral_banner_editor/"+encodeURIComponent(JSON.stringify(this.state.bannerBeans[rowID])));
                            break;
                            {/*case 1:
